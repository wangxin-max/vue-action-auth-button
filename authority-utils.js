...
/**
 * 判断当前路由是否有按钮权限
 * @param currentPermissionId 当前路由需要的按钮权限
 * @returns {boolean}
 */
function hasButtonAuthority (currentPermissionId, buttonOperation) {
  ...
  if (user) {
    ...
    const hasAuth = permissions.find(item => item === currentPermissionId || item.id === currentPermissionId)
...
export { filterMenu, hasAuthority, hasButtonAuthority }
